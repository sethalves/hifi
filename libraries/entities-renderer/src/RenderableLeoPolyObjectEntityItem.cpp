//
//  RenderableLeoPolyObjectEntityItem.cpp
...
gpu::PipelinePointer RenderableLeoPolyObjectEntityItem::_pipeline = nullptr;
...
/*
  ...
  In RenderableLeoPolyObjectEntityItem::render, these flags are checked and changes are propagated along the chain.
...
namespace render {
    ...
    template <> const Item::Bound payloadGetBound(const LeoPolyObjectPayload::Pointer& payload) {
        if (payload && payload->_owner) {
            auto polyVoxEntity = std::dynamic_pointer_cast<RenderableLeoPolyObjectEntityItem>(payload->_owner);
            bool success;
...
EntityItemPointer RenderableLeoPolyObjectEntityItem::factory(const EntityItemID& entityID, const EntityItemProperties& properties) {
    EntityItemPointer entity{ new RenderableLeoPolyObjectEntityItem(entityID) };
    entity->setProperties(properties);
    std::static_pointer_cast<RenderableLeoPolyObjectEntityItem>(entity)->initializePolyVox();
    return entity;
}

RenderableLeoPolyObjectEntityItem::RenderableLeoPolyObjectEntityItem(const EntityItemID& entityItemID) :
    LeoPolyObjectEntityItem(entityItemID),
...
RenderableLeoPolyObjectEntityItem::~RenderableLeoPolyObjectEntityItem() {
    withWriteLock([&] {
        if (_volData) {
            delete _volData;
        }
...
void RenderableLeoPolyObjectEntityItem::initializePolyVox() {
    setVoxelVolumeSize(_voxelVolumeSize);
}

bool RenderableLeoPolyObjectEntityItem::isEdged(PolyVoxEntityItem::PolyVoxSurfaceStyle surfaceStyle) {
    switch (surfaceStyle) {
...
void RenderableLeoPolyObjectEntityItem::setVoxelSurfaceStyle(PolyVoxSurfaceStyle voxelSurfaceStyle) {
    ...
    withWriteLock([&] {
        ...
        bool wasEdged = false;// isEdged(_voxelSurfaceStyle);
        bool willBeEdged = false;// isEdged(voxelSurfaceStyle);
...
glm::vec3 RenderableLeoPolyObjectEntityItem::getSurfacePositionAdjustment() const {
    glm::vec3 result;
    //withReadLock([&] {
...
glm::mat4 RenderableLeoPolyObjectEntityItem::voxelToLocalMatrix() const {
...
glm::mat4 RenderableLeoPolyObjectEntityItem::localToVoxelMatrix() const {
    glm::mat4 localToModelMatrix = glm::inverse(voxelToLocalMatrix());
    return localToModelMatrix;
}

glm::mat4 RenderableLeoPolyObjectEntityItem::voxelToWorldMatrix() const {
    glm::mat4 rotation = glm::mat4_cast(getRotation());
    glm::mat4 translation = glm::translate(getPosition());
    return translation * rotation * voxelToLocalMatrix();
}

glm::mat4 RenderableLeoPolyObjectEntityItem::worldToVoxelMatrix() const {
    glm::mat4 worldToModelMatrix = glm::inverse(voxelToWorldMatrix());
    return worldToModelMatrix;
}

bool RenderableLeoPolyObjectEntityItem::setVoxel(int x, int y, int z, uint8_t toValue) {
    if (_locked) {
        return false;
    }
...
void RenderableLeoPolyObjectEntityItem::forEachVoxelValue(quint16 voxelXSize, quint16 voxelYSize, quint16 voxelZSize,
...
bool RenderableLeoPolyObjectEntityItem::setAll(uint8_t toValue) {
    bool result = false;
    if (_locked) {
        return result;
    }
...
bool RenderableLeoPolyObjectEntityItem::setCuboid(const glm::vec3& lowPosition, const glm::vec3& cuboidSize, int toValue) {
    bool result = false;
    if (_locked) {
        return result;
    }
...
bool RenderableLeoPolyObjectEntityItem::setVoxelInVolume(glm::vec3 position, uint8_t toValue) {
    if (_locked) {
        return false;
    }
...
bool RenderableLeoPolyObjectEntityItem::setSphereInVolume(glm::vec3 center, float radius, uint8_t toValue) {
    bool result = false;
    if (_locked) {
        return result;
    }
...
bool RenderableLeoPolyObjectEntityItem::setSphere(glm::vec3 centerWorldCoords, float radiusWorldCoords, uint8_t toValue) {
    bool result = false;
    if (_locked) {
        return result;
    }
...
bool RenderableLeoPolyObjectEntityItem::findDetailedRayIntersection(const glm::vec3& origin, const glm::vec3& direction,
...
PolyVox::RaycastResult RenderableLeoPolyObjectEntityItem::doRayCast(glm::vec4 originInVoxel,
                                                              glm::vec4 farInVoxel,
...
// virtual
ShapeType RenderableLeoPolyObjectEntityItem::getShapeType() const {
    return SHAPE_TYPE_COMPOUND;
}

void RenderableLeoPolyObjectEntityItem::updateRegistrationPoint(const glm::vec3& value) {
...
bool RenderableLeoPolyObjectEntityItem::isReadyToComputeShape() {
...
void RenderableLeoPolyObjectEntityItem::computeShapeInfo(ShapeInfo& info) {
...
void RenderableLeoPolyObjectEntityItem::setXTextureURL(QString xTextureURL) {
...
void RenderableLeoPolyObjectEntityItem::setYTextureURL(QString yTextureURL) {
...
void RenderableLeoPolyObjectEntityItem::setZTextureURL(QString zTextureURL) {
...
bool RenderableLeoPolyObjectEntityItem::addToScene(EntityItemPointer self,
                                             std::shared_ptr<render::Scene> scene,
                                             render::PendingChanges& pendingChanges) {
    _myItem = scene->allocateID();

    auto renderItem = std::make_shared<LeoPolyObjectPayload>(getThisPointer());
    auto renderData = LeoPolyObjectPayload::Pointer(renderItem);
    auto renderPayload = std::make_shared<LeoPolyObjectPayload::Payload>(renderData);
...
void RenderableLeoPolyObjectEntityItem::removeFromScene(EntityItemPointer self,
                                                  std::shared_ptr<render::Scene> scene,
                                                  render::PendingChanges& pendingChanges) {
...
glm::vec3 RenderableLeoPolyObjectEntityItem::voxelCoordsToWorldCoords(glm::vec3& voxelCoords) const {
...
glm::vec3 RenderableLeoPolyObjectEntityItem::worldCoordsToVoxelCoords(glm::vec3& worldCoords) const {
...
glm::vec3 RenderableLeoPolyObjectEntityItem::voxelCoordsToLocalCoords(glm::vec3& voxelCoords) const {
    return glm::vec3(voxelToLocalMatrix() * glm::vec4(voxelCoords, 0.0f));
}

glm::vec3 RenderableLeoPolyObjectEntityItem::localCoordsToVoxelCoords(glm::vec3& localCoords) const {
    return glm::vec3(localToVoxelMatrix() * glm::vec4(localCoords, 0.0f));
}


void RenderableLeoPolyObjectEntityItem::setVoxelVolumeSize(glm::vec3 voxelVolumeSize) {
...
bool RenderableLeoPolyObjectEntityItem::inUserBounds(const PolyVox::SimpleVolume<uint8_t>* vol,
...
uint8_t RenderableLeoPolyObjectEntityItem::getVoxel(int x, int y, int z) {
    uint8_t result;
    withReadLock([&] {
...
uint8_t RenderableLeoPolyObjectEntityItem::getVoxelInternal(int x, int y, int z) {
...
bool RenderableLeoPolyObjectEntityItem::setVoxelInternal(int x, int y, int z, uint8_t toValue) {
...
bool RenderableLeoPolyObjectEntityItem::updateOnCount(int x, int y, int z, uint8_t toValue) {
...
void RenderableLeoPolyObjectEntityItem::compressVolumeDataAndSendEditPacket() {
...
EntityItemPointer RenderableLeoPolyObjectEntityItem::lookUpNeighbor(EntityTreePointer tree, EntityItemID neighborID, EntityItemWeakPointer& currentWP) {
...
void RenderableLeoPolyObjectEntityItem::cacheNeighbors() {
...
void RenderableLeoPolyObjectEntityItem::copyUpperEdgesFromNeighbors() {
    ...
    if (currentXPNeighbor) {
        auto polyVoxXPNeighbor = std::dynamic_pointer_cast<RenderableLeoPolyObjectEntityItem>(currentXPNeighbor);
    ...
    if (currentYPNeighbor) {
        auto polyVoxYPNeighbor = std::dynamic_pointer_cast<RenderableLeoPolyObjectEntityItem>(currentYPNeighbor);
    ...
    if (currentZPNeighbor) {
        auto polyVoxZPNeighbor = std::dynamic_pointer_cast<RenderableLeoPolyObjectEntityItem>(currentZPNeighbor);
...
void RenderableLeoPolyObjectEntityItem::computeShapeInfoWorker() {
    ...
    QtConcurrent::run([entity, voxelSurfaceStyle, voxelVolumeSize, mesh] {
        ...
        AABox box;
        glm::mat4 vtoM = std::static_pointer_cast<RenderableLeoPolyObjectEntityItem>(entity)->voxelToLocalMatrix();
...
void RenderableLeoPolyObjectEntityItem::setCollisionPoints(ShapeInfo::PointCollection pointCollection, AABox box) {
...
void RenderableLeoPolyObjectEntityItem::setXNNeighborID(const EntityItemID& xNNeighborID) {
    if (xNNeighborID == _id) { // TODO loops are still possible
        return;
    }

    if (xNNeighborID != _xNNeighborID) {
        LeoPolyObjectEntityItem::setXNNeighborID(xNNeighborID);
        cacheNeighbors();
    }
}

void RenderableLeoPolyObjectEntityItem::setYNNeighborID(const EntityItemID& yNNeighborID) {
    if (yNNeighborID == _id) { // TODO loops are still possible
        return;
    }

    if (yNNeighborID != _yNNeighborID) {
        LeoPolyObjectEntityItem::setYNNeighborID(yNNeighborID);
        cacheNeighbors();
    }
}

void RenderableLeoPolyObjectEntityItem::setZNNeighborID(const EntityItemID& zNNeighborID) {
    if (zNNeighborID == _id) { // TODO loops are still possible
        return;
    }

    if (zNNeighborID != _zNNeighborID) {
        LeoPolyObjectEntityItem::setZNNeighborID(zNNeighborID);
        cacheNeighbors();
    }
}

void RenderableLeoPolyObjectEntityItem::setXPNeighborID(const EntityItemID& xPNeighborID) {
    if (xPNeighborID == _id) { // TODO loops are still possible
        return;
    }
    if (xPNeighborID != _xPNeighborID) {
        LeoPolyObjectEntityItem::setXPNeighborID(xPNeighborID);
        _volDataDirty = true;
    }
}

void RenderableLeoPolyObjectEntityItem::setYPNeighborID(const EntityItemID& yPNeighborID) {
    if (yPNeighborID == _id) { // TODO loops are still possible
        return;
    }
    if (yPNeighborID != _yPNeighborID) {
        LeoPolyObjectEntityItem::setYPNeighborID(yPNeighborID);
        _volDataDirty = true;
    }
}

void RenderableLeoPolyObjectEntityItem::setZPNeighborID(const EntityItemID& zPNeighborID) {
    if (zPNeighborID == _id) { // TODO loops are still possible
        return;
    }
    if (zPNeighborID != _zPNeighborID) {
        LeoPolyObjectEntityItem::setZPNeighborID(zPNeighborID);
        _volDataDirty = true;
    }
}


void RenderableLeoPolyObjectEntityItem::bonkNeighbors() {
    ...
    if (currentXNNeighbor && currentXNNeighbor->getType() == EntityTypes::PolyVox) {
        auto polyVoxXNNeighbor = std::dynamic_pointer_cast<RenderableLeoPolyObjectEntityItem>(currentXNNeighbor);
        polyVoxXNNeighbor->setVolDataDirty();
    }
    if (currentYNNeighbor && currentYNNeighbor->getType() == EntityTypes::PolyVox) {
        auto polyVoxYNNeighbor = std::dynamic_pointer_cast<RenderableLeoPolyObjectEntityItem>(currentYNNeighbor);
        polyVoxYNNeighbor->setVolDataDirty();
    }
    if (currentZNNeighbor && currentZNNeighbor->getType() == EntityTypes::PolyVox) {
        auto polyVoxZNNeighbor = std::dynamic_pointer_cast<RenderableLeoPolyObjectEntityItem>(currentZNNeighbor);
