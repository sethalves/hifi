...
void RenderableTextEntityItem::render(RenderArgs* args) {
    ...
    Q_ASSERT(getTypeInternal() == EntityTypes::Text);

    static const float SLIGHTLY_BEHIND = -0.005f;
    glm::vec4 textColor = glm::vec4(toGlm(getTextColorXInternal()), 1.0f);
    glm::vec4 backgroundColor = glm::vec4(toGlm(getBackgroundColorXInternal()), 1.0f);
    ...
    Transform transformToTopLeft = getTransformToCenterInternal();
    if (getFaceCameraInternal()) {
        //rotate about vertical to face the camera
        glm::vec3 dPosition = args->_viewFrustum->getPosition() - getPositionInternal();
