...
QDebug& operator<<(QDebug& dbg, const EntityPropertyFlags& f) {
    ...
    result = f.getHasProperty(PROP_GRAB_GRABBABLE) ? result + "grabGrabbable " : result;
    result = f.getHasProperty(PROP_GRAB_KINEMATIC) ? result + "grabKinematic " : result;
    result = f.getHasProperty(PROP_GRAB_FOLLOWS_CONTROLLER) ? result + "grabFollowsController " : result;
    result = f.getHasProperty(PROP_GRAB_TRIGGERABLE) ? result + "grabTriggerable " : result;
    result = f.getHasProperty(PROP_GRAB_EQUIPPABLE) ? result + "grabEquippable " : result;
    result =
        f.getHasProperty(PROP_GRAB_LEFT_EQUIPPABLE_POSITION_OFFSET) ? result + "grabLeftEquippablePositionOffset " : result;
    result =
        f.getHasProperty(PROP_GRAB_LEFT_EQUIPPABLE_ROTATION_OFFSET) ? result + "grabLeftEquippableRotationOffset " : result;
    result =
        f.getHasProperty(PROP_GRAB_RIGHT_EQUIPPABLE_POSITION_OFFSET) ? result + "grabRightEquippablePositionOffset " : result;
    result =
        f.getHasProperty(PROP_GRAB_RIGHT_EQUIPPABLE_ROTATION_OFFSET) ? result + "grabRightEquippableRotationOffset " : result;
