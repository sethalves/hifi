...
controller::Pose MyAvatar::getControllerPoseInSimulationFrame(controller::Action action) const {
    auto pose = getControllerPoseInSensorFrame(action);
    if (pose.valid) {
        return pose.transform(getSensorToSimulationMatrix());
    } else {
        return controller::Pose(); // invalid pose
    }
}
...
void MyAvatar::prepareForPhysicsSimulation() {
    ...
    if (parentID != QUuid() && parentID != physicsEngine->getID()) {
        ...
        if (success) {
            ...
            glm::vec3 childPosition = getLocalPosition();
...
glm::mat4 MyAvatar::FollowHelper::postPhysicsUpdate(const MyAvatar& myAvatar, const glm::mat4& currentBodyMatrix) {
    if (isActive()) {
        ...
        glm::mat4 sensorToSimulationMatrix = myAvatar.getSensorToSimulationMatrix();
        glm::mat4 simulationToSensorMatrix = glm::inverse(sensorToSimulationMatrix);

        glm::vec3 sensorLinearDisplacement = transformVectorFast(simulationToSensorMatrix, SFLinearDisplacement);
        glm::quat sensorAngularDisplacement = glmExtractRotation(simulationToSensorMatrix) *
            SFAngularDisplacement * glmExtractRotation(sensorToSimulationMatrix);
...
void MyAvatar::handleZoneChange() {
    ...
    if (simulationID != _currentZoneID) {
        ...
        glm::vec3 beforeVelocity = getWorldVelocity();
        glm::vec3 beforeAngularVelocity = getWorldAngularVelocity();
        if (success) {
            ...
            setWorldVelocity(beforeVelocity);
            setWorldAngularVelocity(beforeAngularVelocity);
        } else {
