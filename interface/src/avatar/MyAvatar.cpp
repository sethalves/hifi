...
void MyAvatar::reset() {
    ...
    worldBodyPos.y = getAbsolutePosition().y;

    // XXX make a getParentInverseTransform
    Transform zoneTransform = getParentTransform();
    glm::mat4 zoneInverse;
    zoneTransform.getInverseMatrix(zoneInverse);

    setLocalPosition(glm::vec3(zoneInverse * glm::vec4(worldBodyPos, 1.0f)));
    setLocalOrientation(extractRotation(zoneInverse) * worldBodyRot);
...
void MyAvatar::updateFromHMDSensorMatrix(const glm::mat4& hmdSensorMatrix) {
    ...
    if (!_straighteningLean && (capsuleCheck(diff, STRAIGHTENING_LEAN_CAPSULE_LENGTH, STRAIGHTENING_LEAN_CAPSULE_RADIUS) || hmdIsAtRest)) {
        ...
        if (_straighteningLeanAlpha >= 1.0f) {
            ...
            glm::vec3 pos = lerp(getLocalPosition(), worldBodyPos, _straighteningLeanAlpha);
            glm::quat rot = glm::normalize(safeMix(getLocalOrientation(), worldBodyRot, _straighteningLeanAlpha));
