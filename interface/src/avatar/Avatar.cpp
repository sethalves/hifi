...
void Avatar::simulate(float deltaTime) {
    ...
    bool inViewFrustum = qApp->getViewFrustum()->sphereInFrustum(getAbsolutePosition(), boundingRadius) !=
        ViewFrustum::OUTSIDE;
...
void Avatar::renderBillboard(RenderArgs* renderArgs) {
    ...
    glm::vec3 cameraVector = glm::inverse(rotation) * (qApp->getCamera()->getPosition() - getAbsolutePosition());
...
glm::vec3 Avatar::getDisplayNamePosition() const {
    ...
    if (getSkeletonModel().getNeckPosition(namePosition)) {
        ...
        DEBUG_VALUE("_position =", getAbsolutePosition());
        DEBUG_VALUE("billboardSize =", billboardSize);
        namePosition = getAbsolutePosition() + bodyUpDirection * (billboardSize * HEAD_PROPORTION);
    }
