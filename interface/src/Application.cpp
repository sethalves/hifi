        ...
        _lastFaceTrackerUpdate(0)
{
    ...
    connect(audioIO.data(), &AudioClient::muteEnvironmentRequested, [](glm::vec3 position, float radius) {
        ...
        auto myAvatarPosition = DependencyManager::get<AvatarManager>()->getMyAvatar()->getAbsolutePosition();
        ...
        if (_myCamera.getMode() == CAMERA_MODE_FIRST_PERSON) {
            ...
            if (isHMDMode()) {
                ...
                _myCamera.setPosition(myAvatar->getDefaultEyePosition()
                    ...
                    + (myAvatar->getAbsoluteOrientation() * glm::quat(glm::vec3(0.0f, _rotateMirror, 0.0f))) *
                    glm::vec3(0.0f, 0.0f, -1.0f) * MIRROR_FULLSCREEN_DISTANCE * _scaleMirror
                    + (myAvatar->getAbsoluteOrientation() * glm::quat(glm::vec3(0.0f, PI + _rotateMirror, 0.0f))) * hmdOffset);
            } else {
                _myCamera.setRotation(myAvatar->getWorldAlignedOrientation()
...
void Application::updateMyAvatarLookAtPosition() {
    ...
    if (eyeTracker->isTracking() && (isHMD || eyeTracker->isSimulating())) {
        ...
        if (isHMD) {
            ...
            lookAtSpot = _myCamera.getPosition() + myAvatar->getAbsoluteOrientation() * (hmdRotation * lookAtPosition);
        } else {
        ...
        if (lookingAt && myAvatar != lookingAt.get()) {
            ...
            if (isHMD) {
                ...
                lookAtSpot = _myCamera.getPosition() +
                    myAvatar->getAbsoluteOrientation() * (headRotation * glm::vec3(0.0f, 0.0f, -TREE_SCALE));
            } else {
                lookAtSpot = myAvatar->getHead()->getEyePosition() +
...
const glm::vec3& Application::getAvatarPosition() const {
    return getMyAvatar()->getAbsolutePosition();
}
...
void Application::renderRearViewMirror(RenderArgs* renderArgs, const QRect& region, bool billboard) {
    ...
    if (billboard) {
        ...
        _mirrorCamera.setPosition(myAvatar->getAbsolutePosition() +
                                  myAvatar->getAbsoluteOrientation() * glm::vec3(0.0f, 0.0f, -1.0f) *
                                  BILLBOARD_DISTANCE * myAvatar->getScale());

    } else if (!AvatarInputs::getInstance()->mirrorZoomed()) {
        _mirrorCamera.setPosition(myAvatar->getChestPosition() +
                                  myAvatar->getAbsoluteOrientation() * glm::vec3(0.0f, 0.0f, -1.0f) *
                                  MIRROR_REARVIEW_BODY_DISTANCE * myAvatar->getScale());
        ...
        _mirrorCamera.setPosition(myAvatar->getDefaultEyePosition() +
                                  myAvatar->getAbsoluteOrientation() * glm::vec3(0.0f, 0.0f, -1.0f) *
                                  MIRROR_REARVIEW_DISTANCE * myAvatar->getScale());
    }
...
void Application::emulateMouse(Hand* hand, float click, float shift, int index) {
    ...
    if (isHMDMode()) {
        ...
        glm::vec3 direction = glm::inverse(getMyAvatar()->getAbsoluteOrientation()) * palm->getFingerDirection();
